 ...
 *
 * 第2步 位置环 Kp（此时速度环 Kp=0，先单独跑）：
 *   现象：追得动但慢 -> 加：0.1 -> 0.15 -> 0.2 -> 0.3
 *   现象：抖 / 冲 / 猛跳 -> 减：0.1 -> 0.05
 *   目标：球能从一侧稳定靠近目标，不原地小抖
 *
 * 第3步 速度环 Kp（只有出现过冲才加）：
 *   现象：冲到目标后过冲、来回摆 -> 0.1 -> 0.2 -> 0.3，
 *        加到过冲消失就停
 *   注意：加多了刹车压过驱动，表现为推不动、原地抖
 *
 * 第4步 位置环 Ki（只有静差才加）：
 *   现象：停在目标附近但总差几像素到不了位 -> 0.01 -> 0.03
 *        -> 0.05，够用就停，别多加（多了会过冲/振荡）
 *
 * 第5步 Kd 保持 0：速度环已承担阻尼，加 Kd 容易高频抖动
 *
 * ===== 故障速查 =====
 * 电机完全不动 / 只轻微抖：
